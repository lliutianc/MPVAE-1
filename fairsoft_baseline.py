...
def train_fair_demparity_through_regularize(args):
    ...
    if args.penalize_unfair:
        fair_vae_checkpoint_path = os.path.join(
            args.model_dir, f'fair_vae_prior-{hparams}-{args.dist_gamma:.2f}-{args.seed:04d}.pkl')
    else:
        fair_vae_checkpoint_path = os.path.join(
            args.model_dir, f'unfair_vae_prior-{args.dist_gamma:.2f}-{args.seed:04d}.pkl')
