        ...
        if eval_train:
            ...
            with tqdm(range(int(len(data.train_idx) / float(data.batch_size)) + 1), desc='Train VAE') as t:
                ...
                for i in t:
                    ...
                    train_nll_loss += nll_loss * (end - start)
                    train_c_loss += c_loss * (end - start)
                    train_total_loss += total_loss * (end - start)
                ...
                print("********************train********************")
                print(' & '.join([acc, ha, ebf1, maf1, mif1, nll_loss * args.nll_coeff,
        ...
        if eval_valid:
            ...
            with tqdm(
                ...
                for i in t:
                    ...
                    valid_nll_loss += nll_loss * (end - start)
                    valid_c_loss += c_loss * (end - start)
                    valid_total_loss += total_loss * (end - start)
                ...
                if eval_fairness:
                    ...
                    for sensitive in torch.unique(valid_sensitive, dim=0):
                        feats_z_sensitive = valid_feat_z[idxs[valid_sensitive == sensitive]]
                        mean_diffs += np.mean(
